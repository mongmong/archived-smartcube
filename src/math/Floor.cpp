...
namespace smartcube
{
	Floor::Floor(double precision, const std::vector<std::size_t>& columns) :
	...
	void Floor::handle(Input& input, Output& output)
	{
		...
		for (; !rec->eof(); output.push(rec), rec = input.pop())
		{
			...
			for (; iter != _columns.end(); ++iter)
			{
				...
				(*rec)[index] = floor(static_cast<double> (any) / _precision)
						* _precision;
			}
		}
	}
