...
namespace smartcube
{
	...
	void SignApp::defineOptions(Poco::Util::OptionSet& options)
	{
		ConsoleApp::defineOptions(options);
	}

	const std::string SignApp::getUsage() const
	{
		return "[OPTIONS] <column> [<column> ...]";
	}

	const std::string SignApp::getHeader() const
	{
		return "Sign function.";
	}

	int SignApp::main2(const std::vector<std::string>& args)
	{
...
POCO_APP_MAIN(smartcube::SignApp);
